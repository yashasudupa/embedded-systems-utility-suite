...
bool MQTTCommunicationWrapper::ConnectMqttBroker()
{
	...
	try
	{
		...
		if( m_mosq )
		{
			...
			if( rc != MOSQ_ERR_SUCCESS )
			{
				...
				logg.str("");
				logg << "MQTT_Broadsens::MQTTCommunicationWrapper::ConnectMqttBroker()  :  Message : MQTT create connection failed. Error Info : " << mosquitto_strerror( rc );
				m_exceptionLoggerObj->LogError( logg.str() );
			}
			else
			{
				logg.str("");
				logg << "MQTT_Broadsens::MQTTCommunicationWrapper::ConnectMqttBroker()  :  Message : MQTT Connection Established Successfully. ";
		...
		{
			logg.str("");
			logg << "MQTT_Broadsens::MQTTCommunicationWrapper::ConnectMqttBroker()  :  Message : Connection handle not created.. ";
			m_exceptionLoggerObj->LogError( logg.str() );
	...
	{
		logg.str("");
		logg << "MQTT_Broadsens::MQTTCommunicationWrapper::ConnectMqttBroker()  Message : Error code :  " << e.id << " Error Messag : " << e.what();
...
bool MQTTCommunicationWrapper::DiconnectMqttBroker()
{
	...
	try
	{
		if( m_mosq )
		{
			...
			mosquitto_destroy(m_mosq);
			logg.str("");
			logg << "MQTT_Broadsens::MQTTCommunicationWrapper::DiconnectMqttBroker()  :  Message : Disconnect from the MQTT broker. ";
			m_exceptionLoggerObj->LogInfo( logg.str() );
			return true;
		}
		
		logg.str("");
		logg << "MQTT_Broadsens::MQTTCommunicationWrapper::DiconnectMqttBroker()  :  Message : Failed to Disconnect from the MQTT broker. ";
		m_exceptionLoggerObj->LogError( logg.str() );
	}
	catch( nlohmann::json::exception &e )
	{
...
bool MQTTCommunicationWrapper::Publish( std::string payload, std::string topic )
{
	...
	try
	{
		...
		if( m_mosq )
		{
			...
			{
				logg.str("");
				logg << "MQTT_Broadsens::MQTTCommunicationWrapper::Publish()  Message : Error while publishing payload. Error Info : " << mosquitto_strerror(rc);
				m_exceptionLoggerObj->LogException( logg.str() );
			}
			else
			{
				std::cout << "MQTT_Broadsens::Publish data successfully : Payload : " << payload << std::endl;
				std::cout << "MQTT_Broadsens::Publish data successfully : Topic : " << topic << std::endl;
				return true;
			}
		}
		else
		{
			std::cout << "MQTT_Broadsens::MQTTCommunicationWrapper::Publish() : Connection object not created.. " << std::endl;
		}
	}
	catch( nlohmann::json::exception &e )
	{
...
bool MQTTCommunicationWrapper::Subscribe( std::string topic )
{
	...
	try
	{
		if( m_mosq &&  strcmp( topic.c_str(), "") != 0 ) 
		{
			...
			{
				logg.str("");
				logg << "MQTT_Broadsens::MQTTCommunicationWrapper::Subscribe()  Message : Error while subscribing topic. Error Info : " << mosquitto_strerror(rc); 
				m_exceptionLoggerObj->LogError( logg.str() );
			...
			{
				logg.str("");
				logg << "MQTT_Broadsens::MQTTCommunicationWrapper::Subscribe()  Message : Subscribed Successfully.. Topic : " << topic; 
				m_exceptionLoggerObj->LogInfo( logg.str() );
				return true;
			}
		}
...
void MQTTCommunicationWrapper::OnDisconnect( struct mosquitto *mosq, void *obj, int rc )
{
	...
	logg.str("");
	logg << "MQTT_Broadsens::MQTTCommunicationWrapper::OnDisconnect()  Message : Disconnect from the MQTT broker.";
...
void MQTTCommunicationWrapper::OnConnect( struct mosquitto *mosq, void *obj, int reasonCode )
{
	...
	try
	{
		...
		{
			logg.str("");
			logg << "MQTT_Broadsens::MQTTCommunicationWrapper::OnConnect()  Message : Disconnect from the MQTT broker. Error Info : " << mosquitto_connack_string( reasonCode );
		...
		{
			logg.str("");
			logg << "MQTT_Broadsens::MQTTCommunicationWrapper::OnConnect()  Message : Connection established successfully with the MQTT broker.";
			self->m_exceptionLoggerObj->LogInfo( logg.str() );
...
{
	m_dataCB_brsns( (char *)msg->payload, msg->payloadlen, (char *)msg->topic);
}
 ...
 */
void MQTTCommunicationWrapper::SendDataToLocalBrokerCB( std::function<void(char *, std::uint32_t, char *)> cb )
{
	m_dataCB_brsns = cb;
}
