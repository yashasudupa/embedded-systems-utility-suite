...
private:
	...
	void CreateTelemetryJson( nlohmann::json CreateJsonArrays, char datatype, 
    ...
    short ParseData(short wordQuant, std::string propertyName, 
                    std::string grpName, uint16_t wordValues[]);
	...
	void controlcommandsprocess (nlohmann::json payload, char *buffer, std::string control_topic);
...
private:
    ...
    uint16_t m_priviousBlockState;
    
    GROUP_STRUCT *m_grpStructObj1;
    GROUP_STRUCT *m_grpStructObj2;
    GROUP_STRUCT *m_grpStructObj3;
    
	typedef struct AD_AND_EPOCH
	{
		double analogValue;
		std::time_t epoch;
	}AD_AND_EPOCH;
	
	double data_pressure_last_variable=0;
	double data_pressure_latest=0;
	double data_flowmeter_prev=0;
	double data_flowmeter_latest=0;
	//std::thread m_threadObj;
